...
# 1.先对所有的点（values）做merkle tree m。
# 2.挑选随机坐标special x。（用m生成）
# 3.计算每一个x_polys，总共有4096*4。
# 4.计算4096个x方向的多项式在special_x点处的值。（生成column）
# 5.生成column的merkle tree m2
# 6.选择40个y坐标（用m2生成）
# 7.将40个y坐标上的4个value和column上的点生成merkle proof。



#prove_low_degree(l_evaluations, G2, steps * 2, modulus, exclude_multiples_of=extension_factor 8)]
def prove_low_degree(values, root_of_unity, maxdeg_plus_1, modulus, exclude_multiples_of=0):
    ...
    xs = get_power_cycle(root_of_unity, modulus) #xs是单位根组成的圆,x坐标们
    assert len(values) == len(xs)

    # print("??????",len(values)) # 16384

    # Put the values into a Merkle tree. This is the root that the
    # proof will be checked against
    m = merkelize(values)#values是16384个值，在一条4096的多项式上

    # Select a pseudo-random x coordinate
    special_x = int.from_bytes(m[1], 'big') % modulus #用默克尔根生成的随机x坐标，后面用这列进行递归。
    ...
    quarter_len = len(xs)//4 # 4096=16384/4

    # print(quarter_len) 4096
    ...
    # x_polys = f.multi_interp_4(
    #     [[xs[i+4096*j] for j in range(4)] for i in range(4096)],
    #     [[values[i+quarter_len*j] for j in range(4)] for i in range(4096)]
    # )


    # print(len(x_polys))
    # 4096*4 计算4096个x方向的多项式。

    column = [f.eval_quartic(p, special_x) for p in x_polys]  #计算4096个x方向的多项式在special_x点处的值。
    # print(len(column)) 4096

    m2 = merkelize(column)
    # print("kkkkkkkkkkkkkkkkkk",len(m2)) #8192
    ...
    # poly_positions是一个40*4的list，之后又被拉成了160*1
    # print(ys)
    # print(poly_positions)
    # print(len(poly_positions)) 160

    # This component of the proof, including Merkle branches
    o = [m2[1], mk_multi_branch(m2, ys), mk_multi_branch(m, poly_positions)]# 1 40 160
