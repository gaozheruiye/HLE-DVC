...
class Hybrid_mul_polynomial_commitment_scheme:
    ...
    def genAllPartialProof(self):
        leaves = [[0]*self.M]*self.rho
        for i in range(self.rho):
            for j in range(self.M):
                leaves[i][j] = self.Aux[j,i]


        list =[[0]*(2*self.M)]*self.rho
        # print("len:",len(list))
        # print("len:",len(list[0]))
        # 构造rho 棵d-partition tree

        for i in range(4):
            for j in range(8):
                list[i][j+16] = add(list[i][2*j],list[i][2*j+1])
            for j in range(4):
                list[i][j+24] = add(list[i][2*j+16],list[i][2*j+1+16])
            list[i][28] = add(list[i][24],list[i][25])
            list[i][29] = add(list[i][27],list[i][26])
            list[i][30] = add(list[i][28],list[i][29])
        self.partialProofList = [[0]*self.rho]*self.M
        for k in range(self.M):
            for c in range(self.rho):
                self.partialProofList[k][c] = list[c][31-self.binary_list[k][c]*pow(2,c)]
